...
let entryFilesJS = {};
let entryFilesCSS = {};
if (fs.existsSync(loadPath + '/js/')) {
  let jsFilenames = fs.readdirSync(loadPath + '/js/');
  checkFile(jsFilenames);
}
if (fs.existsSync(loadPath + '/scss/')) {
  let scssFilenames = fs.readdirSync(loadPath + '/scss/');
...
function checkFile(filenames) {
  for (let i = 0; i < filenames.length; i++) {
    ...
    if (regexJS) {
      ...
      Object.assign(entryFilesJS, { [rename]: loadPath + '/js/' + fileName });
    }
    if (regexScss) {
      let entryName = fileName.replace('.scss', '');
      Object.assign(entryFilesCSS, { [entryName]: `${loadPath}/scss/${fileName}` });
    }
  }
}
